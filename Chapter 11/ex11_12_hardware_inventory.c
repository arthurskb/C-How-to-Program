...
int main() {
  ...
  while (1) {
    ...
    printf(
        "1 - (re)Initialize a blank hardware.dat(Be aware that this will erase "
...
void inputData(FILE *fPtr) {
  printf(
      "Please, insert the following data: Record Number - Tool Name - Quantity "
      "- Cost\n");
  Tool data;
...
void listTools(FILE *fPtr) {
  ...
  while ((fread(&current, sizeof(Tool), 1, fPtr) == 1)) {
    if (strcmp(current.name, "") != 0)
      printf("%-15u%-25s%-15u%.2lf\n", current.rec, current.name, current.quant,
             current.cost);
  }
}
...
void deleteTool(FILE *fPtr) {
  printf(
      "Please, insert the Record # (Record number Identifier) from the tool "
