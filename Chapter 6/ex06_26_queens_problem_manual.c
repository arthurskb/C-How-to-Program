/*Yeah, it is appealing to try always the smallest elimination number, but the
problem is you do not account for future moves and overlaps, which are essential
to solve this problem. The code below is the last one i made while trying
different approaches for this heuristic. I think the only way to make it work,
would be to implement some kind of backtracking, or "tried" boards to guide the
algorithm into trying different things next. Since it started to get messy, i
will leave this last sample, and go for a better way to solve it(which i think
is the next one).*/
#include <stdio.h>
...
int main() {
  ...
  while (queensOnBoard < 8) {
      ...
      for (size_t j = 0; j < 8; j++) {
        if (board[i][j] == 0) {
          if (counterBoard[i][j] < smallestElimination) {
            ...
            counterminus1b = counterMinus1(board, smallestEliminationX,
                                           smallestEliminationY);
    ...
    printf("%s%d %d\n", "small x and y = ", smallestEliminationX,
           smallestEliminationY);
