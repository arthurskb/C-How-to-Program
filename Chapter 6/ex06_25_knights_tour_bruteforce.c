/*c - Firstly i tried for a full tour which took too much time, so i changed for 30 movements, and still didn't happen in 20minutes. So it takes A LOT OF TIME.

d - Of course the accessibility-heuristic needed more careful study, and was harder to develop. But it provided better results, and took way less computer power to execute.
The accessibility-heuristic approach isn't 100% guaranteed to obtain a full tour, while the brute-force IS, but it would take A LOT of time. 
Pros and cons in comparison are all around: time to develop, time to execute, computer power required, chance to obtain an answer.*/
